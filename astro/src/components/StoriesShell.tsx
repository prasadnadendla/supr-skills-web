...
// keyboard nav, first-visit hint, and slide-restore on reload. Knows nothing
// about Supr Skills specifically — drop in a different `slides` array and it
// works for course intros, onboarding, etc. (See plan §5.5.)

import { useCallback, useEffect, useRef, useState, type ComponentType } from 'react';

export interface SlideDef<P = unknown> {
  id: string;
  Comp: ComponentType<SlideRenderProps & P>;
  duration: number;
...
export function StoriesShell({
  ...
  // Restore last slide on first mount
  useEffect(() => {
    try {
    ...
    <div className="absolute inset-0 flex flex-col">
      ...
      <div className="absolute top-8 inset-x-0 z-40 flex items-center justify-between px-[18px] py-2 pointer-events-none">
        <div className="flex items-center gap-2.5 pointer-events-auto">
          <div
            className="w-7 h-7 rounded-[9px] grid place-items-center shadow-[0_8px_24px_-8px_rgba(111,92,244,0.6)]"
        ...
        <div className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-[0.06em] text-cream/60 pointer-events-auto">
          <span
            className="w-[7px] h-[7px] rounded-full bg-coral pulse-coral"
            style={{ boxShadow: '0 0 12px #FF7A59' }}
          />
          <span>
            Beta · {String(index + 1).padStart(2, '0')}/{String(slides.length).padStart(2, '0')}
          </span>
          <button
            type="button"
            className="w-7 h-7 grid place-items-center rounded-lg text-cream/70 hover:text-cream"
            aria-label="More"
          >
      ...
      {/* Tap zones — 1/3 left for prev, 2/3 right for next */}
      ...
      {hintShown && (
        <div className="absolute bottom-[110px] left-1/2 -translate-x-1/2 z-50 hint-fade inline-flex items-center gap-2 px-3.5 py-2.5 rounded-full bg-[rgba(20,12,38,0.75)] backdrop-blur-md border border-cream/15 text-cream text-xs font-mono tracking-wide pointer-events-none">
          <span
            className="tap-finger w-3.5 h-3.5 rounded-full bg-coral"
            style={{ boxShadow: '0 0 0 4px rgba(255,122,89,0.25)' }}
          />
